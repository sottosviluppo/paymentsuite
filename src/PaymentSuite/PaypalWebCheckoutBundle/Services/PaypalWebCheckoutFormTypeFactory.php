...
class PaypalWebCheckoutFormTypeFactory {
	...
	public function buildForm() {
		...
		$orderAmount = $this->paymentBridge->getOrder()->getAmount()->getAmount();
		...
		$amount = $orderAmount / $divideBy;
		$formBuilder
			->add('item_name_1', 'hidden', [
				'data' => 'Ordine n. ' . $orderId,
			])
			->add('amount_1', 'hidden', [
				'data' => $amount,
			])
			->add('quantity_1', 'hidden', [
				'data' => '1',
			]);
