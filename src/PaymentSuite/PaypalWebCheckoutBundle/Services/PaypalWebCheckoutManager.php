...
class PaypalWebCheckoutManager {
	...
	private function transactionSuccessful($ipnParameters) {
		...
		// inizializzo cURL per chiamare l'url che controlla se l'IPN sia VERIFIED
		$ch = curl_init($ipnNotifyValidateUrl);
		...
		$ipnValidated = ($result == 'VERIFIED');

		if (!$ipnValidated) {
			throw new PaymentOrderNotFoundException(sprintf('Errore ipn validated - ipnValidated: %s', $result));
		}
		...
		if (!amountMatches) {
			throw new PaymentOrderNotFoundException(sprintf('Errore currency - currency %s', $ipnParameters['mc_currency']));
		}

		if (!(strcmp($ipnParameters['payment_status'], 'Completed') === 0)) {
			throw new PaymentOrderNotFoundException(sprintf('Errore status - stato pagamento %s', $ipnParameters['payment_status']));
		}
